...
class WatchtowerClient implements WatchtowerClientInterface
{
    ...
    public function sendQueryTelemetry(array $data, int $timeout = 3): array
    {
        ...
        try {
            ...
            $response = $this->makeRequestWithTimeout('POST', '/api/agent/queries', $payload, $timeout);
            ...
            // Fail silently for query telemetry
            Log::debug('Watchtower query telemetry failed', [
                'error' => $e->getMessage(),
            ]);
     ...
     * {@inheritdoc}
     */
    public function sendJobTelemetry(array $data, int $timeout = 3): array
    {
        if (! $this->isConfigured()) {
            return ['success' => false];
        }
        ...
        try {
            ...
            $response = $this->makeRequestWithTimeout('POST', '/api/agent/jobs', $payload, $timeout);
            ...
            // Fail silently for job telemetry
            Log::debug('Watchtower job telemetry failed', [
                'error' => $e->getMessage(),
            ]);
     ...
     * {@inheritdoc}
     */
    public function sendLogTelemetry(array $data, int $timeout = 3): array
    {
        if (! $this->isConfigured()) {
            return ['success' => false];
        }
        ...
        try {
            ...
            $response = $this->makeRequestWithTimeout('POST', '/api/agent/logs', $payload, $timeout);
            ...
            // Fail silently for log telemetry
            Log::debug('Watchtower log telemetry failed', [
                'error' => $e->getMessage(),
            ]);
